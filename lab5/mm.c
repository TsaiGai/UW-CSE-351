...
void* mm_malloc(size_t size) {
  ...
  // search the free list for a block that is large enough to hold the requested size
  ptr_free_block = search_free_list(req_size);

  // search_free_list() only yields a block that is AT LEAST the request size,
  // therefore it may be possible the heap runs out of space
  if (ptr_free_block == NULL) { // the heap is out of space
	  // attempts to allocate more space
	  request_more_space(req_size);
	  
	  // updates ptr_free_block
	  ptr_free_block = search_free_list(req_size);

	  // if there still remains no space in the heap, exit out the method
  ...
  // updates the block tag
  preceding_block_use_tag = ptr_free_block -> size_and_tags & TAG_PRECEDING_USED;

  // checks to see if the block needs to be split to prevent excessive internal fragmentation
  if ((block_size - req_size) >= MIN_BLOCK_SIZE) {
	  // updates the block's header size and tag
	  ptr_free_block -> size_and_tags = req_size | preceding_block_use_tag;
	  // updates the size_and_tags for the header and footer
	  size_t remaining_fb_size_and_tags = (block_size - req_size) | preceding_block_use_tag;

	  // the block header is modified to create an extra block
	  size_t* remaining_fb_header = UNSCALED_POINTER_ADD(ptr_free_block, req_size);
	  *remaining_fb_header = remaining_fb_size_and_tags;

	  // a footer is modified as well
	  size_t* remaining_fb_footer = UNSCALED_POINTER_ADD(ptr_free_block, block_size - WORD_SIZE);
          *remaining_fb_footer = remaining_fb_size_and_tags;

	  // reinserts extra block into the free list due to splitting
	  insert_free_block((block_info*) remaining_fb_header);
  } else {
	  // updates preceding_block_used tag for the next block
  	  block_info* next_block = (block_info*) UNSCALED_POINTER_ADD(ptr_free_block, block_size);
  	  next_block -> size_and_tags |= preceding_block_use_tag;
  }

  // updates size_and_tags appropriately
...
void mm_free(void* ptr) {
  ...
  // converts the given used block into a free block
  block_to_free = UNSCALED_POINTER_SUB(ptr, WORD_SIZE);

  // updates the size appropriately
  payload_size = SIZE(block_to_free -> size_and_tags);
  // updates size_and_tags appropriately
  block_to_free -> size_and_tags &= ~TAG_USED;

  // updates the current block's size_and_tags to 0 since it is being freed
  // (both header and footer size_and_tags are being updated)
  *((size_t*) UNSCALED_POINTER_ADD(block_to_free, payload_size - WORD_SIZE)) = block_to_free -> size_and_tags;

  // updates the preceding block's TAG_USED to 0 since the current block is being freed
  block_info* preceding_block = UNSCALED_POINTER_SUB(block_to_free, WORD_SIZE);
  preceding_block -> size_and_tags &= ~TAG_USED;

  // reinserts free block into the head of the free list
  insert_free_block(block_to_free);

  // updates the following block for coalescing
  following_block = UNSCALED_POINTER_ADD(block_to_free, payload_size);
  // coalesces preceding and current blocks if necessary
  if (following_block) {
